RSpec.describe "Neovim" do
  ...
  describe xdg_config_path("nvim/reference.vim") do
    let(:generated_config) do
      ...
      command_parts.each_cons(2) do |(first, second)|
        break(second) if first == "-u"
      end or raise "could not find config file in command flags"
    end

    it "is a symlink to the wrapped config generated by Home Manager" do
      expect(subject.realpath).to eq(generated_config)
    end
  end
