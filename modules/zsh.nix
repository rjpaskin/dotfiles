...
let
  inputPaths = lib.pipe inputs [
    (builtins.mapAttrs (_: input: input.outPath))
    (lib.flip builtins.removeAttrs [ "self" ])
  ];

in {
  darwin.environment.shells = [
    "/run/current-system/sw/bin/zsh" # backup, just in case we bork user profile
    "/etc/profiles/per-user/${config.user}/bin/zsh"
  ];
