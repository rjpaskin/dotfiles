...
class RubyGemsDownloadStrategy < AbstractDownloadStrategy
  def fetch
    ohai "Fetching hcl from gem source"
    HOMEBREW_CACHE.cd do
      ENV['GEM_SPEC_CACHE'] = "#{HOMEBREW_CACHE}/gem_spec_cache"
      safe_system RUBY_BIN/"gem", "fetch", "hcl", "--version", resource.version
    end
  end

  def cached_location
    Pathname.new("#{HOMEBREW_CACHE}/hcl-#{resource.version}.gem")
  end
...
class GemHcl < Formula
  def self._remote_version
    with_env("PATH" => PATH.new(RUBY_BIN, ENV["HOMEBREW_PATH"]), "GEM_SPEC_CACHE" => "#{HOMEBREW_CACHE}/gem_spec_cache") do
      Brew::Gem::CLI.fetch_version("hcl")
    end
  end

  url "hcl", :using => RubyGemsDownloadStrategy
  ...
  def install
    ...
    raise "gem install 'hcl' failed with status #{$?.exitstatus}" unless $?.success?

    bin.rmtree if bin.exist?
    bin.mkpath

    brew_gem_prefix = prefix+"gems/hcl-#{version}"

    completion_for_bash = Dir[
                            "#{brew_gem_prefix}/completion{s,}/hcl.{bash,sh}",
                            "#{brew_gem_prefix}/**/{_,-,}hcl{_,-}completion{s,}{.bash,.sh,}"
                          ].first
    ...
    gemspec = Gem::Specification::load("#{prefix}/specifications/hcl-#{version}.gemspec")
